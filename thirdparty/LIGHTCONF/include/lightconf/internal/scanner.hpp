...
//
    struct token
    {
        ...
        { return type == token_type::char_token && char_value == c; }
    ...
    inline std::string token_name(const token& tok)
    {
        switch (tok.type)
        {
            ...
            case token_type::char_token:
                return "'" + std::string(1, tok.char_value) + "'";
            case token_type::comment_token:
                return "comment";
...
//
    inline std::string scanner::expect_identifier()
    {
        ...
        {
            fail("expected " + token_name(token(token_type::identifier_token)) + " but found " +
                 token_name(cur_token()),
                 cur_token().line, cur_token().pos);
        }
        std::string val = cur_token().string_value;
        next_token();
